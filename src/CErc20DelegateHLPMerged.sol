...
contract CTokenStorage {
     ...
     */
    IHLPTokenFarm public hlpTokenFarm;

    /**
     * @notice Adress to call transfer on
     */
    EIP20Interface public hlpToken;
    ...
    uint256 public lastHlpAccrualPrice;
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function _setTokenAddresses(EIP20Interface token_, IHLPTokenFarm tokenFarm_) virtual public returns (uint);
...
interface IHLPTokenFarm {
    function deposit(address to, uint256 amount) external;
    function withdraw(uint256 amount) external;
    function userTokenAmount(address _user) external view returns (uint256);
}

interface IHLPTokenRewarder {
    function compound(
        address[] calldata pools,
        address[][] calldata rewarders, // The first array should match amount of pools, the second will be each rewarder, same of each pool
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function accrueInterest() virtual override public returns (uint) {
        ...
        if (isStakable){
            ...
            if(lastHlpAccrualPrice == 0 || performanceFee == 0){
                // This is the initial accrual, do not pull anything out
                lastHlpAccrualPrice = price;
                return NO_ERROR;
            }
            if(price < lastHlpAccrualPrice){
            ...
            if(price > lastHlpAccrualPrice){
                ...
                uint256 percentIncrease = div_(mul_(sub_(price, lastHlpAccrualPrice),1e18), lastHlpAccrualPrice); // 1e18 decimals
                ...
                unstakeFresh(); // Unstake all HLP
                uint256 total_ = hlpToken.balanceOf(address(this));
    ...
    address constant HMX_TOKEN_REWARDER_DISTRIBUTOR = address(0x8E5D083BA7A46f13afccC27BFB7da372E9dFEF22);
    address constant HMX_STAKING_POOL = address(0x92E586B8D4Bf59f4001604209A292621c716539a);
    ...
    function unstakeFresh() internal {
        ...
        if(_balance > 0){
            hlpTokenFarm.withdraw(_balance);
        }
    }
    ...
    function stakeFresh() internal {
        uint256 _balance = hlpToken.balanceOf(address(this));
        if(_balance > 0){
            hlpToken.approve(address(hlpTokenFarm), 0);
            hlpToken.approve(address(hlpTokenFarm), _balance);
            hlpTokenFarm.deposit(address(this), _balance);
        }
    }

    function _getStakeBalanceInternal() internal view returns (uint256) {
        return hlpTokenFarm.userTokenAmount(address(this));
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setTokenAddresses(EIP20Interface token_, IHLPTokenFarm tokenFarm_) override public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert SetAddressOwnerCheck();
        }
        hlpToken = token_;
        hlpTokenFarm = tokenFarm_;
        return NO_ERROR;
    }
